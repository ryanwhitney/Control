...
/// Helpers for the **legacy (compatibility) transport** only. The streaming
/// transport streams raw AppleScript into a persistent `osascript -i`; the legacy
/// transport instead runs one `osascript` per command, so it wraps the raw
/// AppleScript into a shell command here.
